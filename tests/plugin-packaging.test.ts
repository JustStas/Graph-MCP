...
describe("Graph MCP plugin packaging", () => {
  ...
  test("the Codex manifest uses only supported fields and complete interface metadata", async () => {
    ...
    expect(userInterface).toMatchObject({
      ...
      longDescription:
        "Read and act on approved Microsoft Graph data through a local MCP server. As well as reading, it can send, reply to, and forward mail, move, archive, and delete messages, create and cancel calendar events, post, edit, and delete Teams messages, change your presence and automatic replies, and create, move, and delete OneDrive files.",
  ...
  test("package and both plugin manifest versions are synchronized by the verifier", async () => {
    ...
    await expect(verifyPluginVersions()).resolves.toBe("0.8.1");
  });
});
